...
Under wxMSW, native command and control identifiers are limited to signed
16-bit values, so custom positive IDs used with windows, menu items, or event
handlers must not be greater than @c SHRT_MAX (32767). Larger values can be
interpreted as negative IDs by the native API and can clash with the negative
IDs generated by wxWidgets.
